...
namespace ExpertSystem.Classes_for_work
{
    ...
    public class DisplayQuestions
    {
        ...
        public Dictionary<int,string> questions_by_analysis() //вывод вопросов и фактов с предварительным анализом. Вызывается с каждым выбором
        {
            ...
            string query = "SELECT DISTINCT [Question_fact],[ID_fact] FROM [Facts],[Parcels],[Conclusion] " +
                "WHERE [Facts].[Type_fact] = 0 AND [Parcels].[Label] = 0 AND [Conclusion].[Rule_check] = 0";
            ...
            while (reader.Read())
            {
                lines.Add(Convert.ToInt32(reader[1]), reader[0].ToString()); //cамо добавление вопросов и фактов
